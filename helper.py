...
# load networks
def load_nets(path = 'pck_nets/'):
    ret = []
    for filename in glob.glob(path + '*.pck'):
        with open(filename, 'rb') as fp:
            ret.append((filename, cPickle.load(fp)))
    return ret


# evaluate net ensemble.
def evaluate_nets(nets, valid_data):
    nets = [i[1] for i in nets]
    for j in range(len(nets)):
        sm = 0
        for i in range(len(valid_data)):
            vote = np.argmax(nets[j].feedforward(valid_data[i][0]))
            sm += (vote == valid_data[i][1])
        print(sm, " / ", len(valid_data))
...
class Network(object):
    ...
    def SGD3(self, training_data_full, epochs, mini_batch_size, eta, point,
            lmbda = 0.0,
            evaluation_data_full=None,
            monitor_evaluation_cost=False,
            monitor_evaluation_accuracy=False,
            monitor_training_cost=False,
            monitor_training_accuracy=False):
        training_data = np.array([(i[0][point:point + AUDIO_INC], i[1]) for i in training_data_full])
        if (evaluation_data_full): evaluation_data = np.array([(i[0][point:point + AUDIO_INC], i[1]) for i in evaluation_data_full])

        if (evaluation_data_full): n_data = len(evaluation_data)
        n = len(training_data)
        if (evaluation_data_full): evaluation_cost, evaluation_accuracy = [], []
        training_cost, training_accuracy = [], []
        for j in range(epochs):
            random.shuffle(training_data)
            mini_batches = [
            ...
            if monitor_evaluation_accuracy:
                ...
                print ("Accuracy on evaluation data: {} / {}".format(accuracy, n_data))
            print()
        if (not evaluation_data_full):
            evaluation_cost, evaluation_accuracy = 0, 0
        return evaluation_cost, evaluation_accuracy, \
            training_cost, training_accuracy
    ...
    def total_cost(self, data, lmbda, convert=False):
        ...
        for x, y in data:
            ...
            if convert: y = vectorized_result(y, len(a))
...
def plot_costs(eval_costs, train_costs, num_epochs):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Cost")
    plt.plot(list(range(num_epochs)), eval_costs, 'g', list(range(num_epochs)), train_costs, 'b')
    plt.show()
...
def plot_accuracies(eval_accs, train_accs, num_epochs):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.plot(list(range(num_epochs)), eval_accs, 'g', list(range(num_epochs)), train_accs, 'b')
    plt.show()
